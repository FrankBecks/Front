...
public class CategoryResource {
    ...
    public List<Category> getAllCategories(@RequestParam(required = false) String filter) {
        if ("salesordertest-is-null".equals(filter)) {
            log.debug("REST request to get all Categorys where salesOrderTest is null");
            return StreamSupport
                .stream(categoryRepository.findAll().spliterator(), false)
                .filter(category -> category.getSalesOrderTest() == null)
