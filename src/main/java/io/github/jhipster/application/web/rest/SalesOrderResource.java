...
public class SalesOrderResource {
    ...
    @Timed
    public List<SalesOrder> getAllSalesOrders(@RequestParam(required = false) String filter) {
        if ("message-is-null".equals(filter)) {
            log.debug("REST request to get all SalesOrders where message is null");
            return StreamSupport
                .stream(salesOrderRepository.findAll().spliterator(), false)
                .filter(salesOrder -> salesOrder.getMessage() == null)
    ...
    public ResponseEntity<SalesOrder> getSalesOrder(@PathVariable Long id) {
        ...
        SalesOrder salesOrder = salesOrderRepository.findOneWithEagerRelationships(id);
